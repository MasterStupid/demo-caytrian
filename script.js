...
function initializeApp() {
    ...
    // XÓA DỮ LIỆU CŨ TRONG LOCALSTORAGE KHI LOAD TRANG
    localStorage.removeItem('gratitudeLeaves');
...
function loadLeaves() {
    try {
        ...
        if (saved) {
            ...
            updateUniqueSets();
            console.log(`📦 Loaded ${leaves.length} leaves from localStorage`);
            dbStatus.textContent = '🟢 Đang sử dụng Local Storage';
            ...
            leaves = [];
            console.log('📦 No saved leaves found, starting fresh');
        }
    } catch (e) {
...
function saveToLocalStorage() {
    try {
        localStorage.setItem('gratitudeLeaves', JSON.stringify(leaves));
        console.log(`💾 Saved ${leaves.length} leaves to localStorage`);
    } catch (e) {
...
function addLeaf(name, teacher, message) {
...
function handleFormSubmit(event) {
    ...
    if (studentNameValue && teacherNameValue && messageValue) {
        try {
            const newLeaf = addLeaf(studentNameValue, teacherNameValue, messageValue);
...
async function create125Leaves() {
    ...
    for(let i = 1; i <= 97; i++) {
        ...
        addLeaf(`SV ${i}`, `Giáo viên ${i % 10 + 1}`, `Tri ân ${i}`);
    }
